...
class InvalidBufferPosition : public cpp53::Exception {
public:
...
private:
    static void throwError(const char* msg) {
        cpp53_throw(InvalidBufferPosition, msg);
    }
...
public:
    ...
    void trim(size_t len) {
        if (len > size_) {
            cpp53_throw(OutOfRange, "trimming too large from output buffer");
        }
        size_ -= len;
    }
    ...
    void writeUint8At(uint8_t data, size_t pos) {
        if (pos + sizeof(data) > size_) {
            cpp53_throw(InvalidBufferPosition, "write at invalid position");
        }
        buffer_[pos] = data;
    }
