...
LabelSequence::serialize(void* buf, size_t buf_len) const {
    ...
    if (expected_size > buf_len) {
        cpp53_throw(BadValue, "buffer too short for LabelSequence::serialize");
    }
    ...
    if (!isOutOfRange(offsets_, offsets_ + offsets_len, bp, buf_len) ||
        ...
        cpp53_throw(BadValue, "serialize would break the source sequence");
    }
...
LabelSequence::equals(const LabelSequence& other, bool case_sensitive) const {
    ...
    for (size_t i = 0; i < len; ++i) {
        ...
        if (cpp53::name::internal::maptolower[ch] !=
            cpp53::name::internal::maptolower[other_ch]) {
            return (false);
        }
    }
    return (true);
        ...
        while (count > 0) {
            ...
            if (case_sensitive) {
                ...
                chdiff = static_cast<int>(
                    cpp53::name::internal::maptolower[label1]) -
                    static_cast<int>(
                        cpp53::name::internal::maptolower[label2]);
            }
...
LabelSequence::stripLeft(size_t i) {
    if (i >= getLabelCount()) {
        cpp53_throw(OutOfRange, "Cannot strip to zero or less labels; " << i <<
...
LabelSequence::getHash(bool case_sensitive) const {
    ...
    while (length > 0) {
        const uint8_t c = *s++;
        hash_combine(hash_val, case_sensitive ? c :
                            cpp53::name::internal::maptolower[c]);
        --length;
    }
    return (hash_val);
}
...
LabelSequence::toText(bool omit_final_dot) const {
    ...
    while (np != np_end) {
        ...
        if (count <= Name::MAX_LABELLEN) {
            ...
            cpp53_throw(BadLabelType, "unknown label type in name data");
        }
    }
